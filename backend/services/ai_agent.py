...
_FIT_SNIPPET = (
    ...
    "var cw=maxX-minX,ch=maxY-minY,pad=4,s=Math.min((innerWidth-pad*2)/cw,(innerHeight-pad*2)/ch);s=Math.max(.1,s);"
...
def _game_html_uses_legacy_single_stage(html_content: str) -> bool:
    ...
    if not arena_tag:
        return True
    # Version 2 makes every stage self-contained (inline layout/SVG).  The
    # previous class-based stages were vulnerable to theme/reset CSS in HTML
    # exports and LMS iframes, collapsing Escalada and other games into a
    # single vertical line.  Rebuild those stored games during export too.
    if 'data-stage-version="2"' not in lowered and "data-stage-version='2'" not in lowered:
        return True
...
def _build_game_fallback_html(sb_slide: dict, bank_questions: list | None = None) -> str:
    ...
    # Each game has its own visual stage. Previously only the labels changed
    # while every mechanic kept the penalty goal; in non-football games that
    # goal could collapse visually into a single vertical post.
    stage_profiles = {
        "penalty_quest": '''<div class="arena penalty-stage" id="arena" data-stage-version="2" style="height:250px;position:relative;overflow:hidden;border-radius:25px;border:3px solid #dffbf0;background:linear-gradient(#37306b 0 30%,#22a765 30% 100%)">
          <svg viewBox="0 0 960 250" aria-label="Campo de futebol educativo" style="position:absolute;inset:0;width:100%;height:100%;display:block"><defs><pattern id="net" width="18" height="18" patternUnits="userSpaceOnUse"><path d="M0 0L18 18M18 0L0 18" stroke="#fff" stroke-opacity=".22" stroke-width="1"/></pattern></defs><rect x="0" y="0" width="960" height="72" fill="#35275d"/><text x="480" y="43" fill="white" opacity=".85" text-anchor="middle" font-size="25">🙌  🎉  🙌  ⭐  🙌  🎉  🙌</text><rect x="325" y="82" width="310" height="128" fill="url(#net)" stroke="white" stroke-width="7"/><path d="M0 210H960M120 72V250M840 72V250" stroke="#fff" stroke-opacity=".35" stroke-width="2"/></svg><div id="keeper" class="keeper" style="position:absolute;left:452px;top:132px;font-size:47px">🧤</div><div id="ball" class="ball" style="position:absolute;left:467px;bottom:18px;font-size:34px;z-index:3">⚽</div><div class="stage-caption" style="position:absolute;left:16px;bottom:10px;padding:6px 11px;border-radius:99px;background:#071126dd;color:#e0f2fe;font-size:11px;font-weight:800">Converta conhecimento em gols</div></div>''',
        "quiz_show": '''<div class="arena quiz-stage" id="arena" data-stage-version="2" style="height:250px;position:relative;overflow:hidden;border-radius:25px;border:3px solid #c4b5fd;background:linear-gradient(145deg,#4c1d95,#111827)"><svg viewBox="0 0 960 250" aria-label="Palco de quiz show" style="position:absolute;inset:0;width:100%;height:100%;display:block"><defs><linearGradient id="beam" x1="0" y1="0" x2="0" y2="1"><stop stop-color="#fef08a" stop-opacity=".75"/><stop offset="1" stop-color="#fef08a" stop-opacity="0"/></linearGradient></defs><path d="M150 0L360 235H40Z" fill="url(#beam)"/><path d="M810 0L920 235H600Z" fill="url(#beam)"/><rect x="315" y="150" width="105" height="68" rx="8" fill="#6d28d9" stroke="#c4b5fd" stroke-width="3"/><rect x="428" y="113" width="105" height="105" rx="8" fill="#7c3aed" stroke="#ede9fe" stroke-width="3"/><rect x="541" y="150" width="105" height="68" rx="8" fill="#6d28d9" stroke="#c4b5fd" stroke-width="3"/><text x="368" y="193" fill="white" font-size="30" text-anchor="middle">2</text><text x="480" y="170" fill="white" font-size="38" text-anchor="middle">1</text><text x="594" y="193" fill="white" font-size="30" text-anchor="middle">3</text></svg><div id="keeper" class="keeper" style="position:absolute;right:25px;top:18px;font-size:28px">👏</div><div id="ball" class="ball" style="position:absolute;left:452px;top:40px;font-size:52px">🎤</div><div class="stage-caption" style="position:absolute;left:16px;bottom:10px;padding:6px 11px;border-radius:99px;background:#071126dd;color:#fff;font-size:11px;font-weight:800">Responda antes que as luzes se apaguem</div></div>''',
        "memory_match": '''<div class="arena memory-stage" id="arena" data-stage-version="2" style="height:250px;position:relative;overflow:hidden;border-radius:25px;border:3px solid #99f6e4;background:linear-gradient(145deg,#0f766e,#172554);padding:22px 210px"><div style="height:100%;display:grid;grid-template-columns:repeat(4,1fr);gap:11px"><span style="display:grid;place-items:center;border-radius:13px;background:#4f46e5;border:2px solid #a5b4fc;font-size:29px">?</span><span style="display:grid;place-items:center;border-radius:13px;background:#fff;color:#172033;font-size:29px">🧠</span><span style="display:grid;place-items:center;border-radius:13px;background:#4f46e5;border:2px solid #a5b4fc;font-size:29px">?</span><span style="display:grid;place-items:center;border-radius:13px;background:#fff;color:#172033;font-size:29px">💡</span><span style="display:grid;place-items:center;border-radius:13px;background:#4f46e5;border:2px solid #a5b4fc;font-size:29px">?</span><span style="display:grid;place-items:center;border-radius:13px;background:#fff;color:#172033;font-size:29px">⭐</span><span style="display:grid;place-items:center;border-radius:13px;background:#4f46e5;border:2px solid #a5b4fc;font-size:29px">?</span><span style="display:grid;place-items:center;border-radius:13px;background:#fff;color:#172033;font-size:29px">🎯</span></div><div id="keeper" class="keeper" style="position:absolute;right:22px;top:18px;padding:8px 12px;border-radius:99px;background:#ffffff24;font-size:11px;font-weight:900">COMBINE</div><div id="ball" class="ball" style="position:absolute;left:78px;top:85px;font-size:52px">✨</div><div class="stage-caption" style="position:absolute;left:16px;bottom:10px;padding:6px 11px;border-radius:99px;background:#071126dd;color:#fff;font-size:11px;font-weight:800">Ative pares e fortaleça as conexões</div></div>''',
        "knowledge_climb": '''<div class="arena climb-stage" id="arena" data-stage-version="2" style="height:250px;position:relative;overflow:hidden;border-radius:25px;border:3px solid #bae6fd;background:linear-gradient(#38bdf8 0 58%,#dbeafe 58% 100%)"><svg viewBox="0 0 960 250" aria-label="Montanha da escalada do conhecimento" style="position:absolute;inset:0;width:100%;height:100%;display:block"><defs><linearGradient id="mount" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#64748b"/><stop offset="1" stop-color="#172033"/></linearGradient></defs><circle cx="825" cy="45" r="27" fill="#fde68a"/><path d="M175 250L405 72L510 158L610 24L825 250Z" fill="url(#mount)"/><path d="M405 72L455 114L430 105L406 119L382 105L355 111Z" fill="#fff"/><path d="M610 24L666 90L635 76L609 91L581 76L552 86Z" fill="#fff"/><path d="M290 218C350 185 385 165 423 141S506 113 548 84S591 52 610 42" fill="none" stroke="#fbbf24" stroke-width="7" stroke-dasharray="9 10" stroke-linecap="round"/><g fill="#fbbf24" stroke="#fff" stroke-width="3"><circle cx="304" cy="210" r="10"/><circle cx="391" cy="158" r="10"/><circle cx="468" cy="123" r="10"/><circle cx="548" cy="84" r="10"/><circle cx="610" cy="42" r="10"/></g><path d="M610 40V10L655 24L610 36" fill="#ef4444" stroke="#fff" stroke-width="2"/></svg><div id="keeper" class="keeper" style="position:absolute;left:585px;top:4px;font-size:34px">🏆</div><div id="ball" class="ball" style="position:absolute;left:274px;bottom:23px;font-size:42px;z-index:3">🧗</div><div class="stage-caption" style="position:absolute;left:16px;bottom:10px;padding:6px 11px;border-radius:99px;background:#071126dd;color:#fff;font-size:11px;font-weight:800">Cada acerto leva você mais perto do cume</div></div>''',
        # SVG + inline layout are intentional here. Some exported courses carry
        # aggressive theme/reset CSS inside the iframe; the former border-only
        # gallows collapsed into one vertical stroke. This self-contained board
        # remains legible in HTML, SCORM and LMS iframe players.
        "word_challenge": '''<div class="arena word-stage" id="arena" data-stage-version="2" style="background:linear-gradient(135deg,#eff6ff,#dbeafe);color:#172033;border-color:#93c5fd;display:grid;grid-template-columns:38% 62%;align-items:center;padding:18px 28px;gap:22px">
          <div class="word-illustration" style="height:205px;border-radius:20px;background:#fff;box-shadow:0 12px 28px #1e3a5f24;display:grid;place-items:center">
            <svg viewBox="0 0 260 190" role="img" aria-label="Forca educativa" style="width:92%;height:92%;display:block">
              <path d="M28 166H150M58 166V25H172V48" fill="none" stroke="#334155" stroke-width="9" stroke-linecap="round" stroke-linejoin="round"/>
              <path d="M172 48V68" stroke="#334155" stroke-width="6" stroke-linecap="round"/>
              <circle class="hangman-part" data-part="1" cx="172" cy="86" r="18" fill="#fef2f2" stroke="#ef4444" stroke-width="6"/>
              <path class="hangman-part" data-part="2" d="M172 104V140" fill="none" stroke="#ef4444" stroke-width="6" stroke-linecap="round"/>
              <path class="hangman-part" data-part="3" d="M172 116L146 129" fill="none" stroke="#ef4444" stroke-width="6" stroke-linecap="round"/>
              <path class="hangman-part" data-part="4" d="M172 116L198 129" fill="none" stroke="#ef4444" stroke-width="6" stroke-linecap="round"/>
              <path class="hangman-part" data-part="5" d="M172 140L151 163" fill="none" stroke="#ef4444" stroke-width="6" stroke-linecap="round"/>
              <path class="hangman-part" data-part="6" d="M172 140L194 163" fill="none" stroke="#ef4444" stroke-width="6" stroke-linecap="round"/>
              <circle cx="225" cy="43" r="24" fill="#4f46e5"/><text x="225" y="50" text-anchor="middle" font-size="20" fill="white">?</text>
            </svg>
          </div>
          <div class="word-board" style="min-width:0;text-align:center">
            <div style="font-size:12px;font-weight:900;letter-spacing:.16em;color:#4338ca;margin-bottom:14px">FORCA DO CONHECIMENTO</div>
            <div id="hangmanStatus" style="font-size:23px;font-weight:900;color:#1e293b">Nenhum erro</div>
            <div class="mistake-dots" aria-label="Progresso dos erros"><i></i><i></i><i></i><i></i><i></i><i></i></div>
            <p style="margin:16px auto 0;max-width:330px;color:#475569;font-size:13px;line-height:1.45;font-weight:700">Responda às questões abaixo. Cada resposta incorreta acrescenta uma parte ao desenho.</p>
          </div>
          <div id="keeper" class="keeper" style="display:none">DICA</div><div id="ball" class="ball" style="display:none">✏️</div>
        </div>''',
        "target_challenge": '''<div class="arena target-stage" id="arena" data-stage-version="2" style="height:250px;position:relative;overflow:hidden;border-radius:25px;border:3px solid #67e8f9;background:radial-gradient(circle at center,#164e63,#081426)"><svg viewBox="0 0 960 250" aria-label="Alvo educativo" style="position:absolute;inset:0;width:100%;height:100%;display:block"><g transform="translate(480 122)"><circle r="104" fill="#f8fafc"/><circle r="82" fill="#2563eb"/><circle r="61" fill="#f8fafc"/><circle r="40" fill="#ef4444"/><circle r="18" fill="#fbbf24"/><text y="7" text-anchor="middle" font-size="19" font-weight="900" fill="#172033">100</text></g><path d="M105 122H340" stroke="#fbbf24" stroke-width="7" stroke-dasharray="12 9"/><path d="M340 122L302 101V143Z" fill="#fbbf24"/></svg><div id="keeper" class="keeper" style="position:absolute;right:20px;top:18px;padding:8px 12px;border-radius:99px;background:#ffffff24;font-size:11px;font-weight:900">PRECISÃO</div><div id="ball" class="ball" style="position:absolute;left:82px;top:92px;font-size:48px;color:#fbbf24">➤</div><div class="stage-caption" style="position:absolute;left:16px;bottom:10px;padding:6px 11px;border-radius:99px;background:#071126dd;color:#fff;font-size:11px;font-weight:800">Analise, escolha e acerte o centro</div></div>''',
    }
    stage_markup = stage_profiles[mechanic]
    mechanic_css = r'''
.stage-caption{position:absolute;left:16px;bottom:10px;padding:6px 11px;border-radius:99px;background:#071126c7;color:#e0f2fe;font-size:11px;font-weight:800}.quiz-stage{background:linear-gradient(155deg,#4c1d95,#111827)}.spotlight{position:absolute;top:-40px;width:270px;height:300px;background:linear-gradient(#fef08aaa,transparent 75%);clip-path:polygon(46% 0,54% 0,100% 100%,0 100%)}.spotlight.left{left:90px;transform:rotate(12deg)}.spotlight.right{right:90px;transform:rotate(-12deg)}.podium{position:absolute;left:315px;bottom:28px;display:flex;align-items:end;gap:8px}.podium i{display:grid;place-items:center;width:105px;height:68px;background:#6d28d9;border:2px solid #c4b5fd;font-size:25px;font-weight:900;font-style:normal}.podium i:nth-child(2){height:105px;background:#7c3aed}.quiz-stage .ball{left:452px;top:48px;bottom:auto;font-size:52px}.quiz-stage .keeper{left:auto;right:24px;top:20px;font-size:25px}.memory-stage{background:linear-gradient(145deg,#0f766e,#172554)}.memory-grid{position:absolute;inset:24px 205px;display:grid;grid-template-columns:repeat(4,1fr);gap:11px}.memory-grid i{display:grid;place-items:center;border-radius:13px;background:linear-gradient(145deg,#312e81,#7c3aed);border:2px solid #a5b4fc;font-size:27px;font-style:normal;box-shadow:0 8px 20px #0005}.memory-grid i:nth-child(even){background:#f8fafc;color:#172033}.memory-stage .ball{left:80px;top:85px;bottom:auto;font-size:48px}.memory-stage .keeper,.word-stage .keeper,.target-stage .keeper{left:auto;right:20px;top:18px;font-size:11px;padding:8px 12px;border-radius:99px;background:#ffffff20}.climb-stage{background:linear-gradient(#38bdf8 0 55%,#dbeafe 55%)}.mountain{position:absolute;left:245px;bottom:0;width:470px;height:225px;background:linear-gradient(135deg,#64748b,#172033);clip-path:polygon(50% 0,100% 100%,0 100%)}.checkpoints{position:absolute;inset:0}.checkpoints i{position:absolute;width:22px;height:22px;border-radius:50%;background:#fbbf24;box-shadow:0 0 18px #fbbf24}.checkpoints i:nth-child(1){left:330px;top:175px}.checkpoints i:nth-child(2){left:405px;top:135px}.checkpoints i:nth-child(3){left:495px;top:105px}.checkpoints i:nth-child(4){left:565px;top:145px}.checkpoints i:nth-child(5){left:468px;top:30px}.climb-stage .ball{left:320px;bottom:38px;font-size:43px}.climb-stage .keeper{left:455px;top:8px;font-size:40px}.word-stage{background:linear-gradient(145deg,#f8fafc,#dbeafe);color:#172033}.letter-preview span{display:grid;place-items:center;width:32px;height:30px;border-radius:8px;background:#fff;border:1px solid #c7d2fe;color:#334155;font-size:11px;font-weight:900;box-shadow:0 3px 8px #1e3a5f18}.target-stage{background:radial-gradient(circle at center,#164e63,#081426)}.target-rings{position:absolute;left:375px;top:19px;width:212px;height:212px;border-radius:50%;background:radial-gradient(circle,#fbbf24 0 14%,#ef4444 14% 31%,#f8fafc 31% 48%,#2563eb 48% 66%,#f8fafc 66%);box-shadow:0 0 45px #22d3ee55}.target-rings b{position:absolute;inset:0;display:grid;place-items:center;color:#172033}.target-stage .ball{left:92px;top:92px;bottom:auto;font-size:53px;color:#fbbf24}.arena:not(.penalty-stage) .ball.shoot-left,.arena:not(.penalty-stage) .ball.shoot-right{animation:mechanicSuccess .75s ease forwards}.arena:not(.penalty-stage) .keeper.dive-left,.arena:not(.penalty-stage) .keeper.dive-right{transform:none}@keyframes mechanicSuccess{50%{transform:translate(160px,-55px) scale(1.35) rotate(18deg)}100%{filter:drop-shadow(0 0 22px #fbbf24)}}
 .hangman-part{opacity:0;transition:opacity .28s ease,filter .28s ease}.hangman-part.visible{opacity:1;filter:drop-shadow(0 0 5px #ef444466)}.mistake-dots{display:flex;justify-content:center;gap:8px;margin-top:18px}.mistake-dots i{width:13px;height:13px;border-radius:50%;background:#cbd5e1;border:2px solid #94a3b8;transition:.25s}.mistake-dots i.active{background:#ef4444;border-color:#b91c1c;box-shadow:0 0 10px #ef444477}
'''
...
const MAX_LIVES=__MAX_LIVES__;
const Game={round:0,xp:0,coins:0,lives:MAX_LIVES,combo:0,correct:0,locked:false,start(){start.classList.add('hidden');play.classList.remove('hidden');this.sync();this.next()},restart(){this.round=0;this.xp=0;this.coins=0;this.lives=MAX_LIVES;this.combo=0;this.correct=0;finish.classList.add('hidden');play.classList.remove('hidden');this.sync();this.next()},revealPenalty(){const mistakes=MAX_LIVES-this.lives;document.querySelectorAll('.hangman-part').forEach((part,index)=>part.classList.toggle('visible',index<mistakes));document.querySelectorAll('.mistake-dots i').forEach((dot,index)=>dot.classList.toggle('active',index<mistakes));const status=document.getElementById('hangmanStatus');if(status)status.textContent=mistakes===0?'Nenhum erro':mistakes+' de '+MAX_LIVES+' erros'},sync(){lives.textContent=this.lives;combo.textContent=this.combo;xp.textContent=this.xp;coins.textContent=this.coins;bar.style.width=(this.round/questionBank.length*100)+'%';this.revealPenalty()},next(){this.locked=false;ball.className='ball';keeper.className='keeper';feedback.textContent='';if(this.round>=questionBank.length||this.lives<=0)return this.end();this.sync();const q=questionBank[this.round];question.textContent=q.question;answers.innerHTML='';q.alternatives.map((text,index)=>({text,index,key:Math.random()})).sort((a,b)=>a.key-b.key).forEach(a=>{const b=document.createElement('button');b.className='answer';b.textContent=a.text;b.onclick=()=>this.answer(q,a.index,b);answers.appendChild(b)})},answer(q,index,button){if(this.locked)return;this.locked=true;const ok=QuestionEngine.validateAnswer(q,index),side=Math.random()>.5?'right':'left';ball.classList.add('shoot-'+side);if(ok){keeper.classList.add('dive-'+(side==='left'?'right':'left'));this.combo++;this.correct++;const gain=100+(this.combo*25);this.xp+=gain;this.coins+=10*this.combo;button.classList.add('correct');feedback.textContent='⚽ ACERTO! +'+gain+' XP · '+q.explanation;this.particles()}else{keeper.classList.add('dive-'+side);this.combo=0;this.lives--;button.classList.add('wrong');feedback.textContent='⚠️ Resposta incorreta: uma parte foi acrescentada. '+q.explanation}QuestionEngine.saveResult({questionId:q.id,correct:ok,xp:this.xp,at:Date.now()});this.round++;this.sync();setTimeout(()=>this.next(),1600)},particles(){for(let i=0;i<32;i++){const p=document.createElement('i');p.className='particle';p.style.left='50%';p.style.top='40%';p.style.background=['#22d3ee','#a78bfa','#fbbf24','#34d399'][i%4];p.style.setProperty('--x',(Math.random()*420-210)+'px');p.style.setProperty('--y',(Math.random()*260-130)+'px');arena.appendChild(p);setTimeout(()=>p.remove(),1100)}},end(){play.classList.add('hidden');finish.classList.remove('hidden');const accuracy=Math.round(this.correct/questionBank.length*100);finishTitle.textContent=this.lives>0?'Desafio concluído!':'Treino encerrado — tente de novo!';achievement.textContent=accuracy===100?'🌟 CONQUISTA: MESTRE INVICTO':accuracy>=60?'🥉 CONQUISTA: ESPECIALISTA':'🎯 MISSÃO: TREINAR NOVAMENTE';finalXp.innerHTML=this.xp+'<small>XP total</small>';finalCoins.innerHTML=this.coins+'<small>moedas</small>';finalAccuracy.innerHTML=accuracy+'%<small>precisão</small>';QuestionEngine.saveResult({complete:true,xp:this.xp,coins:this.coins,accuracy})}};
</script></body></html>'''
    return (
        template.replace("__TITLE__", safe_title)
        .replace("__QUESTIONS__", questions_json)
        .replace("__MAX_LIVES__", "6" if mechanic == "word_challenge" else "3")
        .replace(
            '<div class="arena" id="arena"><div class="crowd"></div><div class="goal"></div><div id="keeper" class="keeper">🧤</div><div id="ball" class="ball">⚽</div></div>',
            stage_markup,
        )
        .replace("</style>", mechanic_css + "</style>", 1)
