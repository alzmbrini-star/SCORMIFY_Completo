...
- **[DONE]** Fix deployment health check timeout - all 3 startup events now non-blocking (asyncio.create_task). App ready in ~1s instead of 30-60s
- **[DONE]** Fix nginx startup failure in production ARM64 - fix_nginx_modules.sh v3: removes broken symlinks, replaces nginx-code-server.conf Lua-free, replaces sites-enabled/default with app proxy + /health endpoint on port 80
