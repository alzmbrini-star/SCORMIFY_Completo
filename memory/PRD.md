...
- **Apply Media Changes to Existing Project (2026-03-03)**: Fixed "Editar Mídia" flow — now shows "Aplicar Alterações ao Projeto" button (blue) instead of "Gerar Curso". New endpoint POST /api/agent/sessions/{id}/apply-media-changes updates existing project slides with backgrounds, text colors, animations, and narration config WITHOUT regenerating the course. Redirects back to Editor after applying. TESTED & VALIDATED.
