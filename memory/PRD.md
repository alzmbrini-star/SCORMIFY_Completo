...
- 2026-03-30: FIX - Agent courses endpoint only showing agent-created courses in production. Root cause: old code had `createdByAgent: True` filter. Fixed: removed filter, increased limit to 500, added `agentSessionId` fallback for source detection, eliminated N+1 query (was doing separate find_one per project). Now returns all projects correctly.
