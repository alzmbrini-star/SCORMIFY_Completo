...
const getAssetUrl = (src) => {
  ...
  if (src.startsWith('http')) {
    // Whiteboards generated by older builds were sometimes saved with the
    // static frontend origin.  They are API media and must always be served
    // by the current backend, just like project/global assets.
    const whiteboardMatch = src.match(/https?:\/\/[^/]+(\/api\/whiteboard\/file\/[^?#]+(?:[?#].*)?)/);
    if (whiteboardMatch) {
      return `${API_URL}${whiteboardMatch[1]}`;
    }
