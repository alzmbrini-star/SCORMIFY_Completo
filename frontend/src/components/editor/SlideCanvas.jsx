                  ...
                  {element.flipbookType === 'external' && element.flipbookUrl ? (
                    ...
                    <div className="w-full h-full overflow-auto" style={{ pointerEvents: 'none', background: 'transparent' }} data-testid={`pdf-pages-${element.id}`}>
                      {element.pdfPages.map((p, i) => (
                        <img key={i} src={getAssetUrl(p)} alt={`Página ${i + 1}`} className="w-full h-auto block" style={{ marginBottom: 8, borderRadius: 6 }} />
                      ))}
                    </div>
                  ) : element.flipbookType === 'pdf' && element.flipbookUrl ? (
                    <iframe
                      src={getAssetUrl(element.flipbookUrl) + (element.pdfDisplay === 'clean' ? '#toolbar=0&navpanes=0&scrollbar=0' : '')}
                      className="w-full h-full border-0"
                      title="PDF Viewer"
                      style={{ pointerEvents: 'none' }}
                    />
