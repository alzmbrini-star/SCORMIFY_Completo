...
/**
 * Conversational chat for Media Config (step 5). Tweaks per-slide
 * image source, avatar settings, background colors etc. via natural
 * language BEFORE applying the actual generation.
 */
export default function MediaConfigChat({ sessionId, onMediaConfigUpdate }) {
  ...
  const suggestions = [
    'Use Leonardo em todos os slides',
    'Troca o avatar de todos os slides para mulher',
    'Desabilita narracao no slide 1',
    'Fundo azul escuro nos slides 2 ao 4',
  ];
  ...
  const send = useCallback(async (text) => {
    ...
    try {
      ...
      const res = await fetch(`${API}/api/agent/sessions/${sessionId}/media-chat`, {
        method: 'POST',
      ...
      if (data.ops && data.ops.length > 0) {
        ...
        if (onMediaConfigUpdate) {
          onMediaConfigUpdate({ mediaConfig: data.mediaConfig, bgConfig: data.bgConfig });
        }
      }
    } catch (e) {
      setMessages(prev => [...prev, {
    ...
    setSending(false);
  }, [input, sending, sessionId, messages, onMediaConfigUpdate]);

  return (
    <div className="flex flex-col h-full" data-testid="media-config-chat">
      <div className="px-3 py-2 border-b border-slate-800 flex items-center gap-2">
        <Sparkles className="w-4 h-4 text-violet-400" />
        <span className="text-sm font-semibold text-white">Chat com Agente IA</span>
        <Badge className="bg-fuchsia-900/30 text-fuchsia-300 text-[9px] ml-auto">Media Config</Badge>
      </div>
          ...
          {messages.length === 0 ? (
            <div className="space-y-3">
              <p className="text-xs text-slate-400 leading-relaxed">
                Ajuste configuracoes de midia em linguagem natural. Sugestoes:
              </p>
              <div className="space-y-1.5">
                {suggestions.map((s, i) => (
                  <button
                    ...
                    data-testid={`media-chat-suggestion-${i}`}
                  >
                    {s}
                  </button>
                ))}
              ...
              <p className="text-[10px] text-slate-500 mt-2 leading-relaxed">
                Nota: imagens/audios so sao gerados quando voce clica em <strong>&quot;Gerar Midia&quot;</strong>.
              </p>
            </div>
          ) : (
            messages.map((m, i) => (
      ...
      <div className="p-3 border-t border-slate-800 bg-slate-900/50">
        <div className="flex gap-2">
          ...
          <Button
            ...
            data-testid="media-chat-send"
          >
            {sending ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Send className="w-3.5 h-3.5" />}
          </Button>
        </div>
        <p className="text-[9px] text-slate-500 mt-1.5">
          Enter envia, Shift+Enter quebra linha. Indices 1-based.
        </p>
      </div>
    </div>
  );
