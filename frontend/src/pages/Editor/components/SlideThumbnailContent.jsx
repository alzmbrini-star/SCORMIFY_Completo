      ...
      {elements.map((el) => {
        ...
        if (el.type === 'html') {
          ...
          const srcDoc = isFullDoc
            ? wrapInteractiveFullbleed(raw, el.htmlDisplayMode || 'page')
            : `<html><head><style>html,body{margin:0;padding:0;overflow:hidden;background:transparent;}</style></head><body>${raw}</body></html>`;
          return (
